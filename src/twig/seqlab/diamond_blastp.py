...
KWARGS = dict(
    ...
    description=textwrap.dedent("""
        ...
        | common workflows easily available in twig. The default out  |
        | format is outfmt-6 + corrected_bitscore qlen slen. You can use  |
        | --outfmt to set to standard outfmt-6, or --outfmt ... to list   |
        | custom fields in this format.                                   |
        -------------------------------------------------------------------
    """),
    epilog=textwrap.dedent("""
        Examples
        --------
        $ diamond-blastp data1.faa data2.faa > data1_data2.diamond.tsv
        $ diamond-blastp data1.faa data1.db -db > data1_data2.diamond.tsv
        $ diamond-blastp data1.faa data2.faa -t 10 -d /tmp -e 1e-5 > 1_2.tsv
        $ diamond-blastp data1.faa data2.faa -t 10 -d /tmp -e 1e-5 --outfmt > 1_2.tsv
        $ diamond-blastp data1.faa data2.faa -t 10 -d /tmp -e 1e-5 --outfmt qseqid sseqid evalue > 1_2.tsv
        $ diamond-blastp data1.faa data2.db -db --kwargs-blastp '--taxonlist 33090' ... > 1_2.tsv
    """)
)
...
def get_parser_diamond_blastp(parser: ArgumentParser | None = None) -> ArgumentParser:
    ...
    # add arguments
    parser.add_argument("query", type=Path, help="fasta protein sequence to align to the target")
    parser.add_argument("target", type=Path, help="fasta protein sequence or diamond db file (w/ .db suffix) as target")
    ...
    parser.add_argument("-t", "--threads", type=int, metavar="int", default=4, help="number of threads per diamond job; default=4")
    ...
    parser.add_argument("-db", "--database", action="store_true", help="target file is already a diamond indexed database")
